    ...
    <nav className={`fixed w-full z-50 transition-all duration-300 ${scrolled ? 'bg-white shadow-md py-2' : 'bg-transparent py-4'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          {/* Logo */}
          <a href="#" onClick={scrollToTop} className="flex-shrink-0 flex items-center gap-2 group">
          ...
          <div className="hidden md:flex items-center space-x-8">
            {NAV_LINKS.map((link) => (
              <a
                ...
                className={`${
                  link.highlight 
                    ? 'bg-brand-pink hover:bg-brand-pinkDark text-white px-5 py-2 rounded-full font-semibold transition-colors shadow-lg cursor-pointer'
                    : `font-medium hover:text-brand-pink transition-colors cursor-pointer ${scrolled ? 'text-slate-700' : 'text-white/90'}`
                }`}
              >
                {link.name}
              </a>
          ...
          <div className="px-4 pt-2 pb-6 space-y-2">
            {NAV_LINKS.map((link) => (
              <a
                ...
                href={link.href}
                onClick={(e) => handleNavClick(e, link.href)}
                className={`block px-3 py-4 rounded-md text-base font-medium text-center cursor-pointer ${
                  link.highlight
